...
from llmcode.prompts import orchestrator_prompt, project_name, role_prompt
...
def test_project_name_is_cwd_basename():
    assert project_name("/Users/x/Projects/apps/llmcode") == "llmcode"
    assert project_name("/Users/x/Projects/apps/llmcode/") == "llmcode"
    assert project_name("/tmp/myproj") == "myproj"


def test_orchestrator_prompt_injects_cwd_and_project():
    p = orchestrator_prompt(has_memory_tool=False, workspace="/Users/x/apps/llmcode")
    assert "/Users/x/apps/llmcode" in p          # the working directory
    assert "Project name: llmcode" in p          # the derived project name
...
def test_memory_project_rule_only_with_memory_tool_and_workspace():
    ...
    p = orchestrator_prompt(has_memory_tool=True, workspace="/a/b/llmcode")
    assert "PROJECT SCOPE" in p
    assert 'kyp_session_create(project="llmcode"' in p
    assert 'kyp_project_context(project="llmcode"' in p
    assert 'write notes under the "llmcode/..." path' in p
    # no memory tool => no kyp-mem scope rule even with a workspace.
    p2 = orchestrator_prompt(has_memory_tool=False, workspace="/a/b/llmcode")
...
def test_role_prompt_injects_workspace_for_subagents():
    coder = role_prompt("coder", has_memory_tool=True, workspace="/a/b/llmcode")
    assert "Project name: llmcode" in coder
    assert "PROJECT SCOPE" in coder  # writing role + memory tool
    explorer = role_prompt("explorer", workspace="/a/b/llmcode")
    assert "Project name: llmcode" in explorer
